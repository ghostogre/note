...
15. 类似下图的优惠券：绝大多数方案（比如京东）都是采取直接切图实现背景，例如京东是切了两个圆形图片（使用i标签，两个小圆都是云图片，为什么不直接整个背景都是图片，可能是小圆圈图片比较小加载资源更快。我们的项目里，优惠券背景还有其他文字，所以直接切成一张图减少加载次数）产生缺口的效果。因为即使CSS能实现，兼容性也很难保证。
...
3. **背景图问题：**优惠券这类背景存在不透明的小缺口，而且容器本身还有图片背景，还得考虑到小程序兼容性问题。
   ...
   - **方案三**：直接把圆形缺口的容器背景图切出来，作为伪元素，这样就看上去和透明的一样了。然后背景直接可以使用CSS实现，小圆缺口的图比较小也不会有加载问题。**缺点**就是要保证和容器背景图不会出现错位。只适合移动端。（否则一行多列，需要切的图就会很多了，除非背景是纯色，纯色可以无视这些问题）
