...
20. 渲染页面大致相同的时候会复用节点，导致可能节点上出现不该存在的class。
...
17. 想要隐藏滚动条最好的方法是外部包裹一层`overflow:hidden`，然后高度低于滚动容器的高度，这样就能截掉滚动条的显示。
...
19. **useRequest**
    ...
    -  **防抖 节流**：内部是使用Lodash的防抖节流方法，但是在我们给支付宝修改的版本里，我们需要自己写防抖节流。

20. **条纹加载条**：使用 `linear-gradient`可以设置条纹加载样式（渐变实际上是设置background-image，可以配合background-color设置背景色，渐变做蒙版 - 使用`rgba(255, 255, 255, .3)`做渐变蒙版），假如需要背景色也渐变，可以在加一层，在父级元素上设置渐变背景，在子元素上设置渐变蒙版。
