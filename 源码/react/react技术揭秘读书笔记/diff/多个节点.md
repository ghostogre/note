...
> **为什么filbert不直接使用双链表呢？**
>
> 答：使用链表插入删除更加迅速，内存利用率高。市面上单链表使用大于双链表，但是双链表多一个指针，追求时间效率的话不是很合适。
...
接下来**遍历**剩余的`newChildren`，通过`newChildren[i].key`就能在`existingChildren`中找到`key`相同的`oldFiber`。

## 标记节点是否移动

我们需要确定是哪个节点移动导致了后面无法复用，需要我们移动的是那个节点。

既然我们的目标是寻找移动的节点，那么我们需要明确：节点是否移动是以什么为参照物？

我们的参照物是：最后一个可复用的节点在`oldFiber`中的位置索引（用变量`lastPlacedIndex`表示）。

由于本次更新中节点是按`newChildren`的顺序排列。在遍历`newChildren`过程中，每个`遍历到的可复用节点`一定是当前遍历到的`所有可复用节点`中**最靠右的那个**。

那么我们只需要比较`遍历到的可复用节点`在上次更新时是否也在`lastPlacedIndex`对应的`oldFiber`后面，就能知道两次更新中这两个节点的顺序有没有改变。

我们用变量`oldIndex`表示`遍历到的可复用节点`在`oldFiber`中的位置索引。如果`oldIndex < lastPlacedIndex`，代表本次更新该节点需要向右移动。

`lastPlacedIndex`初始为`0`，每遍历一个可复用的节点，如果`oldIndex >= lastPlacedIndex`，则`lastPlacedIndex = oldIndex`。

单纯文字表达比较晦涩，提供两个Demo，可以对照着理解。
...
===第二轮遍历开始===
newChildren === cdb，没用完，不需要执行删除旧节点（用完的话，也就是要删除旧的节点）
oldFiber === bcd，没用完，不需要执行插入新节点（newChildren没用完，oldFiber用完了，那就是需要插入新节点了）
...
key === c的key 在 oldFiber中存在
const oldIndex = c（之前）.index; // fiber对象上保存了index
